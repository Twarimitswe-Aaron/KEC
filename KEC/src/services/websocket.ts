...
export interface WebSocketEvents {
  ...
  "message:deleted": (data: { chatId: number; messageId: number }) => void;
...
class WebSocketService {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on("disconnect", (reason) => {
        ...
        if (reason === "io server disconnect") {
          console.log(
            "🚫 [WebSocket] Server initiated disconnect - not reconnecting"
          );
          return;
        }
  ...
  on<K extends keyof WebSocketEvents>(event: K, callback: WebSocketEvents[K]) {
    if (this.socket) {
      ...
      console.warn(
        `⚠️ [WebSocket] Cannot register listener for ${
          event as string
