...
import { useGetCourseForStudentQuery } from '../state/api/courseApi'
...
const UserLessonsView = () => {
  ...
  const { data, isLoading, error } = useGetCourseForStudentQuery(courseId, { skip: !courseId });
    ...
    <div className="">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className='my-4'>
            ...
            <p className="text-md text-gray-600 mt-2 max-w-2xl">
              Access your unlocked lessons and resources. Quizzes will be graded automatically.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {lessons.map((lesson: any) => (
          <div key={lesson.id} className="bg-white rounded-xl shadow p-5 border border-gray-200">
            ...
            {(lesson.resources || []).length > 0 && (
              <div className="mt-3 space-y-2">
                {lesson.resources.map((r: any) => (
                  <div key={r.id} className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
                    ...
                    <div>
                      {r.type === 'quiz' && r.form?.quizzes?.length > 0 ? (
                        r.form.quizzes.map((q: any) => (
                          <button
                            key={q.id}
                            onClick={() => navigate(`/dashboard/course/${courseId}/quiz/${q.id}?lessonId=${lesson.id}&formId=${r.form.id}`)}
